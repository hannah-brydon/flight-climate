...
server <- function(input, output) {
  ...
  d_airline <- reactive({
      #browser()
      # Create flights that will be plotted
      d_flights %>%
          filter(
              airline_name == input$airline_name,
              date == Sys.Date() - 30
              ) %>%
          mutate(color = ifelse(start_country == "New Zealand", "red","blue"))
  })

  paths <- reactive({
      paths <- gcIntermediate(
          d_airline() %>% select(start_lng, start_lat) %>% as.matrix(),
          d_airline() %>% select(dest_lng, dest_lat) %>% as.matrix(),
          n = 100,
          addStartEnd = T,
          sp = TRUE,
